...
When a conversation concerns the `logic/` layer, also read:

`logic/AGENTS.md`

The nested guidance takes priority over the general mathematical guidance below for work in `logic/`. `THEORY_ATLAS.md` is the authoritative mathematical ground source for both general and logic-layer work.

Use the Theory Atlas as the go-to mathematical orientation and as the primary reference for mathematical claims, distinctions, invariants, and validity. Keep it in the background unless mathematics is relevant to the current question. Consult other detailed theory files only when the Atlas explicitly points outward, leaves a specific issue unresolved, or a precise result needs verification. If another theory note conflicts with the Atlas, treat the Atlas as authoritative unless the user explicitly says otherwise.
...
- Identify weak assumptions, contradictions, hidden tradeoffs, and premature abstractions.
...
- Keep discussion grounded in the current repository and the next concrete consumer.
...
- Do not derive the total scope or final package architecture from the Theory Atlas. For the `logic/` layer, however, faithfully preserve the mathematical structure of each concept that is actually chosen for implementation, as required by `logic/AGENTS.md`.
...
Avoid commitments until a current requirement, experiment, or consumer makes them necessary.

- Do not design distant architectural layers merely because they may eventually be useful.
- Ask what consumes a proposed component and whether that consumer exists yet.
- Point out when work is proceeding bottom-up from an abstraction with no current use.
- Prefer small experiments, sketches, and vertical slices that answer present questions.
- Recognize that a speculative outline may still be useful as a map for choosing a starting point and imagining possible next steps.
- Treat names, interfaces, module boundaries, type hierarchies, and examples as provisional while they are still being explored.
- Do not describe a proposed design as settled unless the user has accepted it or existing code genuinely depends on it.
- When proposing an experiment, state what question it tests and which parts remain disposable.

Preserving optionality does not mean avoiding every concrete choice. Make the smallest decision needed to learn something or support the current consumer. Do not extend that decision further than the evidence warrants.
...
The `logic/` layer is the explicit exception described in `logic/AGENTS.md`: there, the mathematics determines the canonical conceptual model for each implemented mathematical concept, while Python determines its faithful representation.
...
The project is partly a way for the user to learn Python. Assume limited exposure to Python's available mechanisms, not limited ability to reason about them.
...
- Start from the next concrete behavior or consumer.
